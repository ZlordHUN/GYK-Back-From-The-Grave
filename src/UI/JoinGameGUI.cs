...
namespace GraveyardKeeperCoop.UI
{
    ...
    public class JoinGameGUI : BaseMenuGUI
    {
        ...
        private void CreateBackButton(Transform parent, UIFont font)
        {
            ...
            backButtonObj = CreateVisibleButton(buttonParent, template, font, "btn_back", "Back", new Vector3(0, BOTTOM_BUTTON_Y, 0), out backButtonWidget);
            ...
            // Layout (centered around Back at x=0): [Refresh]  [Back]  [Connect]
            refreshButtonObj = CreateVisibleButton(buttonParent, template, font, "btn_refresh", "Refresh", new Vector3(-SIDE_BUTTON_OFFSET, BOTTOM_BUTTON_Y, 0), out refreshButtonWidget);
        ...
        private void ConfigureGamepadNavigation()
        {
            ...
            for (int i = 0; i < tabs.Length; i++)
            {
                ...
                tab.SetCustomDirectionItem(firstRow ?? refreshNavigationItem, Direction.Down);
            }
            ...
            for (int i = 0; i < rows.Length; i++)
            {
                ...
                row.SetCustomDirectionItem(i + 1 < rows.Length ? rows[i + 1] : refreshNavigationItem, Direction.Down);
        ...
        private void ApplyResolutionLayout()
        {
            ...
            {
                backButtonObj.transform.localPosition = new Vector3(0f, buttonY, 0f);
                SetButtonWidgetDepth(backButtonObj, JOIN_BUTTON_WIDGET_DEPTH);
            }

            if (refreshButtonObj != null)
            {
                refreshButtonObj.transform.localPosition = new Vector3(-layout.SideButtonOffset, buttonY, 0f);
                SetButtonWidgetDepth(refreshButtonObj, JOIN_BUTTON_WIDGET_DEPTH);
            }

            if (connectButtonObj != null)
            {
                connectButtonObj.transform.localPosition = new Vector3(layout.SideButtonOffset, buttonY, 0f);
                SetButtonWidgetDepth(connectButtonObj, JOIN_BUTTON_WIDGET_DEPTH);
            }

            CoopMod.Logger.LogInfo(
                $"[JoinGameGUI] Applied layout {layout.Name}: " +
                $"browser={layout.ServerBrowserPosition}, buttonsY={buttonY}, " +
        ...
        private new void Update()
        {
            ...
            // Escape leaves the menu, same as the Back button (issue #8: players reported being
            // stuck here with no way out). Matches the existing behaviour of ModsListGUI.
