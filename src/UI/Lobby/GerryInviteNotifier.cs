...
namespace GraveyardKeeperCoop.UI
{
    ...
    public class GerryInviteNotifier : MonoBehaviour
    {
        ...
        private bool showResponseOptions;
        ...
        public void ShowInvitation(string inviterName, System.Action onAcceptCallback, System.Action onDeclineCallback)
        {
            ShowNotification(
                inviterName,
                $"{inviterName} has invited you to their graveyard!",
                true,
                onAcceptCallback,
                onDeclineCallback);
        }
        ...
        /// <summary>
        /// Shows a non-actionable compatibility notice. No Accept/Decline dialogue is
        /// created, because this invitation cannot safely be joined.
        /// </summary>
        public void ShowCompatibilityNotice(string inviterName, string message)
        {
            ShowNotification(inviterName, message, false, null, null);
        }

        private void ShowNotification(
            string inviterName,
            string message,
            bool includeResponseOptions,
            System.Action onAcceptCallback,
            System.Action onDeclineCallback)
        {
            CoopMod.Logger.LogInfo("[GERRY] ========== GerryInviteNotifier.ShowNotification() ==========");
            CoopMod.Logger.LogInfo($"[GERRY] Inviter: {inviterName}");
            CoopMod.Logger.LogInfo($"[GERRY] Actionable: {includeResponseOptions}");
            CoopMod.Logger.LogInfo($"[GERRY] isShowing: {isShowing}");
            CoopMod.Logger.LogInfo($"[GERRY] gerryContainer null? {gerryContainer == null}");

            if (isShowing)
            {
                CoopMod.Logger.LogWarning("[GERRY] Gerry is already showing a notification!");
                return;
            }

            currentInviterName = inviterName;
            currentMessage = message;
            showResponseOptions = includeResponseOptions;
        ...
        /// <summary>
        /// Shows a speech bubble attached to Gerry after a delay. Actionable invitations
        /// continue to MultiAnswer; compatibility notices dismiss without any response UI.
        /// </summary>
        private System.Collections.IEnumerator ShowSpeechBubble(float delay)
        {
            ...
            // Compatibility explanations need longer than the short normal invite line.
            // Any click still dismisses the speech bubble early.
            float elapsed = 0f;
            float maxWait = showResponseOptions ? 1.5f : 10f;
            ...
            {
                CoopMod.Logger.LogInfo("Compatibility notice complete; dismissing Gerry without response options");
