...
namespace GraveyardKeeperCoop.Network
{
    ...
    public class OnlineCoopManager : MonoBehaviour
    {
        ...
        private sealed class RedEyeIntroFormationState
        {
            public PlayerComponent LocalSourcePlayer;
            public PlayerComponent LocalPresentationPlayer;
            public WorldGameObject RedEyeActor;
            public GameObject LocalNameTag;
            public Transform LocalBubbleAnchor;
            public Vector3 OriginalBubbleAnchorLocalPosition;
            public readonly Dictionary<ulong, Transform> DialogueAnchors =
                new Dictionary<ulong, Transform>();
            public readonly Dictionary<ulong, Vector3> SlotPositions =
                new Dictionary<ulong, Vector3>();
            public readonly List<KeyValuePair<Renderer, bool>> HiddenRenderers =
                new List<KeyValuePair<Renderer, bool>>();
            public float StartedAt;
            public float NextActorLookupAt;
            public string ScriptName;
            public string RosterSignature;
            public Vector3 LastLayoutAnchor;
            public bool LayoutLogged;
            public bool PostIntroSlotsCommitted;
        }
        ...
        private const float RedEyeFormationTimeoutSeconds = 120f;
        private const float RedEyeRearRowSpacingMultiplier = 1.15f;
        private const float RedEyeFormationPixelSpacing = 72f;
        private const float RedEyeFormationNameTagOffset = 28f;
        private static readonly Vector3 RedEyePostIntroHouseFacing =
            Vector3.right;
        private RedEyeIntroFormationState redEyeIntroFormation;
        private readonly Dictionary<int, ulong> dialogueNameTagBubbleOwners =
        ...
        /// <summary>
        /// Returns the stable head anchor for a player represented by the active
        /// Red Eye intro formation. Player prefab bubble points belong to animated
        /// frame objects and are not stable enough for a bubble that outlives one
        /// animation frame.
        /// </summary>
        public bool TryGetRedEyeIntroDialogueAnchor(
            CSteamID steamID,
            out Transform anchor)
        {
            anchor = null;
            RedEyeIntroFormationState state = redEyeIntroFormation;
            return state != null &&
                   steamID != CSteamID.Nil &&
                   state.DialogueAnchors.TryGetValue(
                       steamID.m_SteamID,
                       out anchor) &&
                   anchor != null;
        }

        public void SuppressPlayerNameTagForBubble(
            CSteamID steamID,
            int bubbleInstanceId)
        {
            if (steamID == CSteamID.Nil || bubbleInstanceId == 0)
                return;

            if (dialogueNameTagBubbleOwners.TryGetValue(
                    bubbleInstanceId,
                    out ulong previousOwner) &&
                previousOwner != steamID.m_SteamID)
            {
                dialogueNameTagBubbleOwners.Remove(bubbleInstanceId);
                ApplyDialogueNameTagVisibility(new CSteamID(previousOwner));
            }

            dialogueNameTagBubbleOwners[bubbleInstanceId] = steamID.m_SteamID;
            ApplyDialogueNameTagVisibility(steamID);
        }

        public void RestorePlayerNameTagForBubble(int bubbleInstanceId)
        {
            if (!dialogueNameTagBubbleOwners.TryGetValue(
                    bubbleInstanceId,
                    out ulong ownerSteamID))
            {
                return;
            }

            dialogueNameTagBubbleOwners.Remove(bubbleInstanceId);
            ApplyDialogueNameTagVisibility(new CSteamID(ownerSteamID));
        }

        private void ApplyDialogueNameTagVisibility(CSteamID steamID)
        {
            GameObject nameTag = GetPlayerNameTag(steamID);
            if (nameTag == null)
                return;

            bool suppressed = HasActivePlayerSpeechBubble(steamID);
            foreach (ulong ownerSteamID in dialogueNameTagBubbleOwners.Values)
            {
                if (ownerSteamID == steamID.m_SteamID)
                {
                    suppressed = true;
                    break;
                }
            ...
            if (nameTag.activeSelf == suppressed)
                nameTag.SetActive(!suppressed);
        }

        private bool HasActivePlayerSpeechBubble(CSteamID steamID)
        {
            WorldGameObject player = steamID == SteamUser.GetSteamID()
                ? redEyeIntroFormation?.LocalSourcePlayer?.wgo
                : GetRemotePlayer(steamID);
            if (player == null || SpeechBubbleGUI.all == null)
                return false;

            return SpeechBubbleGUI.all.TryGetValue(
                       player.unique_id,
                       out SpeechBubbleGUI bubble) &&
                   bubble != null;
        }

        private GameObject GetPlayerNameTag(CSteamID steamID)
        {
            if (steamID == CSteamID.Nil)
                return null;
            if (steamID == SteamUser.GetSteamID())
                return redEyeIntroFormation?.LocalNameTag;
            if (steamID == remotePlayerSteamID)
                return remotePlayerNameTag;
        ...
        private void ClearDialogueNameTagSuppressions()
        {
            var owners = new HashSet<ulong>(dialogueNameTagBubbleOwners.Values);
            dialogueNameTagBubbleOwners.Clear();
            foreach (ulong ownerSteamID in owners)
                ApplyDialogueNameTagVisibility(new CSteamID(ownerSteamID));
            NameTagManager.RestoreDefaultRenderDepth();
        }
        ...
        public void SetRemotePlayerNameTagTarget(
            CSteamID steamID,
            Transform target)
        {
            if (steamID == CSteamID.Nil ||
            ...
            RetargetPlayerNameTag(steamID, target);
        }

        public void RestoreRemotePlayerNameTagTarget(
            CSteamID steamID,
            Transform expectedTarget)
        {
            GameObject nameTag = GetPlayerNameTag(steamID);
            PlayerNameTag component =
                nameTag != null ? nameTag.GetComponent<PlayerNameTag>() : null;
            if (component == null || expectedTarget == null ||
                component.Target != expectedTarget)
            {
                return;
            }

            PlayerComponent player = GetRemotePlayerComponent(steamID);
            SetRemotePlayerNameTagTarget(
                steamID,
                player?.wgo?.bubble_pos_tf);
        }
        ...
        private static void CloseOpenGameGuiForDisconnect()
        {
            try
            {
                GUIElements elements = GUIElements.me;
                GameGUI gameGui = elements?.game_gui;
                if (gameGui != null &&
                    (gameGui.is_shown || gameGui.gameObject.activeSelf))
                {
                    gameGui.Hide(false);
                    CoopMod.Logger.LogInfo(
                        "[OnlineCoopManager] Closed the open game GUI before disconnecting");
                    return;
                }

                // Defensive fallback for a tab left active without its GameGUI parent.
                MapGUI map = elements?.map;
                if (map != null && (map.is_shown || map.gameObject.activeSelf))
                {
                    map.Hide(false);
                    CoopMod.Logger.LogInfo(
                        "[OnlineCoopManager] Closed an orphaned map GUI before disconnecting");
                }
            }
            catch (System.Exception ex)
            {
                CoopMod.Logger.LogWarning(
                    $"[OnlineCoopManager] Could not close the game GUI during disconnect: {ex.Message}");
            }
        }
        ...
        public void EnableAsHost(CSteamID clientSteamID)
        {
            ...
            FishingPresentationSync.Instance?.EnableSync();
        ...
        public void DisableOnlineCoop()
        {
            ...
            ClearDialogueNameTagSuppressions();
            EndRedEyeIntroFormation("online co-op disabled");
            ...
            FishingPresentationSync.Instance?.DisableSync();
        ...
        private void DisableAIPathfinding()
        {
            ...
            try
            {
                ...
                // LeaveTrailComponent.CustomUpdate also chooses and plays footstep sounds.
                // Those sounds are global rather than spatial, so a remote peer walking on
                // an indoor surface can otherwise replace the local player's outdoor steps.
                if (remotePlayer.Trail != null)
                {
                    remotePlayer.Trail.UpdateTrails = false;
                    CoopMod.Logger.LogInfo("[OnlineCoopManager] Disabled trail and footstep simulation on remote player");
                }
        ...
        /// <summary>
        /// Starts the catalogued Red Eye presentation formation. The real local
        /// player remains at the vanilla script anchor; a noninteractive duplicate
        /// supplies its visible party slot so script state and camera ownership are
        /// not changed.
        /// </summary>
        public void BeginRedEyeIntroFormation(string scriptName)
        {
            if (redEyeIntroFormation != null || !IsOnlineCoopEnabled ||
                GameLoadSync.Instance?.IsInIntroPhase != true)
            {
                return;
            }

            PlayerComponent source = MainGame.me?.player?.GetComponent<PlayerComponent>() ??
                localPlayer;
            if (source == null)
            {
                CoopMod.Logger.LogWarning(
                    "[IntroFormation] Cannot start Red Eye formation: local player is missing");
                return;
            }

            try
            {
                PlayerComponent presentation = PlayerComponent.SpawnPlayer(
                ...
                presentation.gameObject.name =
                    $"IntroPresentationPlayer_{localSteamID.m_SteamID}";
                presentation.transform.position = source.transform.position;

                if (presentation.wgo != null)
                {
                    presentation.wgo.unique_id =
                        -2000000000L - (long)(localSteamID.m_SteamID % 900000000UL);
                    presentation.wgo.data.SetParam("speed", LazyConsts.PLAYER_SPEED);
                    presentation.wgo.data.SetInventorySize(0);
                }

                BaseCharacterComponent character =
                    presentation.wgo?.components?.character;
                if (character != null)
                {
                    character.can_be_locally_controlled = false;
                    character.control_enabled = false;
                    // The vanilla Red Eye flow places the stranger to the right
                    // of the player and explicitly makes the player face right.
                    character.LookAt(Direction.Right);
                    character.OnStopped();
                }

                DisableRemoteSimulation(presentation);
                CameraTools.RemoveFromCameraTargets(presentation.transform, 0f);
                SetLayerRecursive(presentation.gameObject, source.gameObject.layer);

                ChunkedGameObject chunked =
                    presentation.GetComponent<ChunkedGameObject>();
                if (chunked != null)
                    chunked.always_active = true;

                var cosmeticsDriver =
                    presentation.gameObject.AddComponent<PlayerCosmeticsDriver>();
                cosmeticsDriver.Initialize();
                cosmeticsDriver.SetCosmetics(
                    CosmeticsSync.Instance?.LocalCosmetics ?? PlayerCosmetics.Default);

                var state = new RedEyeIntroFormationState
                {
                    LocalSourcePlayer = source,
                    LocalPresentationPlayer = presentation,
                    LocalBubbleAnchor = source.wgo?.bubble_pos_tf,
                    OriginalBubbleAnchorLocalPosition =
                        source.wgo?.bubble_pos_tf != null
                            ? source.wgo.bubble_pos_tf.localPosition
                            : Vector3.zero,
                    StartedAt = Time.realtimeSinceStartup,
                    ScriptName = scriptName
                };
                redEyeIntroFormation = state;
                ...
                for (int i = 0; i < renderers.Length; i++)
                {
                    ...
                    if (renderer == null)
                        continue;
                    state.HiddenRenderers.Add(
                        new KeyValuePair<Renderer, bool>(renderer, renderer.enabled));
                    renderer.enabled = false;
                }

                state.LocalNameTag = NameTagManager.CreateNameTag(
                    presentation.gameObject,
                    NameTagManager.GetSteamDisplayName(localSteamID));
                CopySortingLayer(source, presentation);

                CoopMod.Logger.LogInfo(
                    $"[IntroFormation] Started '{scriptName}' with deterministic party slots");
            }
            catch (System.Exception ex)
            {
                CoopMod.Logger.LogError(
                    $"[IntroFormation] Failed to start Red Eye formation: {ex}");
                EndRedEyeIntroFormation("startup failed");
            }
        }

        public void EndRedEyeIntroFormation(string reason)
        {
            RedEyeIntroFormationState state = redEyeIntroFormation;
            if (state == null)
                return;
            redEyeIntroFormation = null;

            bool committedPostIntroSlots = state.PostIntroSlotsCommitted;
            if (!committedPostIntroSlots &&
                string.Equals(
                    reason,
                    "intro phase ended",
                    System.StringComparison.Ordinal))
            {
                committedPostIntroSlots = CommitRedEyePostIntroSlots(state);
            }

            if (committedPostIntroSlots)
                MaintainRedEyePostIntroHouseFormation(state);

            if (state.LocalBubbleAnchor != null)
            {
                state.LocalBubbleAnchor.localPosition =
                    state.OriginalBubbleAnchorLocalPosition;
            }

            RestoreRedEyeNameTagTargets(state);

            for (int i = 0; i < state.HiddenRenderers.Count; i++)
            {
                Renderer renderer = state.HiddenRenderers[i].Key;
                if (renderer != null)
                    renderer.enabled = state.HiddenRenderers[i].Value;
            }

            if (committedPostIntroSlots)
                ApplyRedEyePostIntroHouseFacing(state.LocalSourcePlayer);

            foreach (Transform anchor in state.DialogueAnchors.Values)
            {
                if (anchor != null)
                    Destroy(anchor.gameObject);
            }
            state.DialogueAnchors.Clear();

            if (state.LocalNameTag != null)
                NameTagManager.DestroyNameTag(state.LocalNameTag);
            if (state.LocalPresentationPlayer != null)
                Destroy(state.LocalPresentationPlayer.gameObject);

            if (!committedPostIntroSlots)
            {
                if (ghostDriver != null && hasReceivedRemotePlayerPosition)
                    ghostDriver.ForcePosition(targetRemotePosition);
                foreach (SecondaryRemotePeer peer in secondaryRemotePeers.Values)
                {
                    if (peer?.Ghost != null && peer.HasPosition)
                        peer.Ghost.ForcePosition(peer.TargetPosition);
                }
            }

            CoopMod.Logger.LogInfo(
                $"[IntroFormation] Ended '{state.ScriptName}' ({reason})");
        }

        /// <summary>
        /// Switches the visible intro party from the dark-zone formation to the
        /// house carpet as soon as vanilla teleports the local player home. The
        /// intro state remains alive for the final dialogue lines and is cleaned
        /// up when the script actually ends.
        /// </summary>
        public void TransitionRedEyeIntroFormationToHouse()
        {
            RedEyeIntroFormationState state = redEyeIntroFormation;
            if (state == null || state.PostIntroSlotsCommitted)
                return;

            if (!CommitRedEyePostIntroSlots(state))
            {
                CoopMod.Logger.LogWarning(
                    "[IntroFormation] Could not apply house layout at gd_player_respawn");
                return;
            }

            MaintainRedEyePostIntroHouseFormation(state);
            CoopMod.Logger.LogInfo(
                "[IntroFormation] Entered house; switched immediately to post-intro carpet layout while dialogue remains active");
        }

        private bool UpdateRedEyeIntroFormation()
        {
            RedEyeIntroFormationState state = redEyeIntroFormation;
            if (state == null)
                return false;
            ...
            if (GameLoadSync.Instance?.IsInIntroPhase != true ||
                state.LocalSourcePlayer == null ||
                MainGame.me?.player != state.LocalSourcePlayer.wgo ||
                Time.realtimeSinceStartup - state.StartedAt >
                    RedEyeFormationTimeoutSeconds)
            {
                EndRedEyeIntroFormation(
                    GameLoadSync.Instance?.IsInIntroPhase == true
                        ? "source changed or timeout"
                        : "intro phase no longer active");
                return false;
            }

            PlayerComponent presentation = state.LocalPresentationPlayer;
            if (presentation == null)
            {
                EndRedEyeIntroFormation("presentation avatar missing");
                return false;
            }

            if (state.PostIntroSlotsCommitted)
            {
                MaintainRedEyePostIntroHouseFormation(state);
                return true;
            }
            ...
            if (state.LocalNameTag == null && GUIElements.me?.hud != null)
            {
                state.LocalNameTag = NameTagManager.CreateNameTag(
                    presentation.gameObject,
                    NameTagManager.GetSteamDisplayName(SteamUser.GetSteamID()));
                ApplyDialogueNameTagVisibility(SteamUser.GetSteamID());
            }

            List<CSteamID> roster = GetDeterministicLobbyRoster();
            if (roster.Count == 0)
                return true;

            string signature = BuildRosterSignature(roster);
            if (state.RosterSignature != signature)
            {
                state.RosterSignature = signature;
                CoopMod.Logger.LogInfo(
                    $"[IntroFormation] Roster slots: {signature}");
            }

            WorldGameObject redEyeActor = ResolveRedEyeActor(state);
            Vector3 anchor = state.LocalSourcePlayer.transform.position;
            Vector3 towardRedEye = Vector3.right;
            if (redEyeActor != null)
            {
                Vector3 delta = redEyeActor.transform.position - anchor;
                delta.z = 0f;
                if (delta.sqrMagnitude > 0.01f)
                    towardRedEye = delta.normalized;
            }

            // Party members stand beside one another across the encounter axis.
            // Additional rows extend away from Red Eye, leaving the front pair
            // centered on the original single-player anchor.
            Vector3 lateral = new Vector3(-towardRedEye.y, towardRedEye.x, 0f);
            float columnSpacing = CalculateRedEyeFormationSpacing();
            float rowSpacing = columnSpacing * RedEyeRearRowSpacingMultiplier;
            CSteamID localSteamID = SteamUser.GetSteamID();
            state.LastLayoutAnchor = anchor;
            int formationPlayerCount = Mathf.Clamp(
                roster.Count,
                1,
                4);

            if (!state.LayoutLogged && redEyeActor != null)
            {
                state.LayoutLogged = true;
                CoopMod.Logger.LogInfo(
                    $"[IntroFormation] Layout anchor={anchor}, RedEye={redEyeActor.transform.position}, " +
                    $"toward={towardRedEye}, spacing={columnSpacing:F1}");
            }

            for (int i = 0; i < roster.Count && i < 4; i++)
            {
                Vector3 slotPosition = anchor +
                    GetRedEyeFormationOffset(
                        i,
                        ...
                        columnSpacing,
                        rowSpacing);
                CSteamID steamID = roster[i];
                state.SlotPositions[steamID.m_SteamID] = slotPosition;
            }

            if (!state.SlotPositions.TryGetValue(
                    localSteamID.m_SteamID,
                    out Vector3 localSlot))
            {
                return true;
            }

            PlaceRedEyePresentationPlayer(
                presentation,
                localSlot,
                null,
                redEyeActor,
            ...
            Transform presentationHead = presentation.wgo?.bubble_pos_tf;
            Vector3 canonicalBubbleOffset = presentationHead != null
                ? presentationHead.position - presentation.transform.position
                : new Vector3(0f, 72f, 0f);

            for (int i = 0; i < roster.Count && i < 4; i++)
            {
                CSteamID steamID = roster[i];
                Vector3 slotPosition = state.SlotPositions[steamID.m_SteamID];
                PlayerComponent representedPlayer;
                if (steamID == localSteamID)
                {
                    representedPlayer = presentation;
                }
                else
                {
                    representedPlayer = GetRemotePlayerComponent(steamID);
                    if (representedPlayer == null)
                        continue;

                    GhostDriver ghost = steamID == remotePlayerSteamID
                        ? ghostDriver
                ...
                Transform dialogueAnchor = UpdateRedEyeDialogueAnchor(
                    state,
                    steamID,
                    slotPosition + canonicalBubbleOffset);
                RetargetPlayerNameTag(steamID, dialogueAnchor);
                ApplyDialogueNameTagVisibility(steamID);
            }

            if (state.LocalBubbleAnchor != null)
            {
                state.LocalBubbleAnchor.position =
                    localSlot + canonicalBubbleOffset;
            }

            ApplyRedEyeFormationNameTagLayout(state, roster);

            return true;
        }

        private bool CommitRedEyePostIntroSlots(
            RedEyeIntroFormationState state)
        {
            if (state == null || state.SlotPositions.Count == 0)
                return false;

            PlayerComponent source = state.LocalSourcePlayer;
            if (source == null)
                return false;

            Vector3 anchorDelta =
                source.transform.position - state.LastLayoutAnchor;
            if (anchorDelta.sqrMagnitude > 0.0001f)
            {
                var slotIds = new List<ulong>(state.SlotPositions.Keys);
                for (int i = 0; i < slotIds.Count; i++)
                {
                    ulong slotId = slotIds[i];
                    state.SlotPositions[slotId] += anchorDelta;
                }
            }

            ApplyPostIntroHouseCarpetLayout(
                state,
                source.transform.position);

            CSteamID localSteamID = SteamUser.GetSteamID();
            if (localSteamID == CSteamID.Nil ||
                !state.SlotPositions.TryGetValue(
                    localSteamID.m_SteamID,
                    out Vector3 localSlot))
            {
                return false;
            ...
            if (source.wgo != null)
                source.wgo.PlaceAtPos(localSlot);
            else
                source.transform.position = localSlot;
            ApplyRedEyePostIntroHouseFacing(source);

            ChunkedGameObject sourceChunked =
                source.GetComponent<ChunkedGameObject>();
            sourceChunked?.RecalculateChunk();

            if (remotePlayerSteamID != CSteamID.Nil &&
                state.SlotPositions.TryGetValue(
                    remotePlayerSteamID.m_SteamID,
                    out Vector3 primaryRemoteSlot))
            {
                targetRemotePosition = primaryRemoteSlot;
                hasReceivedRemotePlayerPosition = true;
                ghostDriver?.ForcePosition(primaryRemoteSlot);
            }

            foreach (KeyValuePair<ulong, SecondaryRemotePeer> entry in
                     secondaryRemotePeers)
            {
                SecondaryRemotePeer peer = entry.Value;
                if (peer == null ||
                    !state.SlotPositions.TryGetValue(
                        entry.Key,
                        out Vector3 remoteSlot))
                {
                    continue;
                }

                peer.TargetPosition = remoteSlot;
                peer.HasPosition = true;
                peer.Ghost?.ForcePosition(remoteSlot);
            }

            CoopMod.Logger.LogInfo(
                $"[IntroFormation] Committed post-intro party slots; local={localSlot}, players={state.SlotPositions.Count}, facing=Right");
            state.PostIntroSlotsCommitted = true;
            return true;
        }

        private void MaintainRedEyePostIntroHouseFormation(
            RedEyeIntroFormationState state)
        {
            if (state == null || !state.PostIntroSlotsCommitted)
                return;
            ...
            if (source != null)
            {
                source.transform.position = localSlot;
                if (source.wgo != null)
                    source.wgo.transform.position = localSlot;
                ApplyRedEyePostIntroHouseFacing(source);
            }
            for (int i = 0; i < state.HiddenRenderers.Count; i++)
            {
            ...
            for (int i = 0; i < roster.Count && i < 4; i++)
            {
                ...
                if (steamID != localSteamID)
                {
                    PlayerComponent remote = GetRemotePlayerComponent(steamID);
                    if (remote == null)
                        continue;
        ...
        private static void ApplyPostIntroHouseCarpetLayout(
            RedEyeIntroFormationState state,
            Vector3 anchor)
        {
            if (state == null)
            ...
            float spacing = CalculateRedEyeFormationSpacing();
            for (int slot = 0;
                 slot < roster.Count && slot < playerCount;
                 slot++)
            {
                state.SlotPositions[roster[slot].m_SteamID] =
                    anchor + GetPostIntroHouseCarpetOffset(slot, spacing);
            }

            CoopMod.Logger.LogInfo(
                $"[IntroFormation] Applied {playerCount}-player post-intro carpet layout");
        }

        private static Vector3 GetPostIntroHouseCarpetOffset(
            int slot,
            float spacing)
        {
            // The vanilla respawn is centered on the carpet. Keep the original
            // upper/lower pair and extend later slots toward the chest at the
            // open end of the carpet.
            switch (slot)
            {
                case 0:
                    return Vector3.down * (spacing * 0.5f);
                case 1:
                    return Vector3.up * (spacing * 0.5f);
                default:
                    return Vector3.down * (spacing * (slot - 0.5f));
            }
        }

        private static void ApplyRedEyePostIntroHouseFacing(
            PlayerComponent player)
        {
            BaseCharacterComponent character =
                player?.wgo?.components?.character;
            if (character == null)
                return;

            Vector2 facing = RedEyePostIntroHouseFacing;
            character.OnStopped();
            character.direction = facing;
            character.LookAt(facing);
        }

        private static Transform UpdateRedEyeDialogueAnchor(
            RedEyeIntroFormationState state,
            CSteamID steamID,
            Vector3 position)
        {
            if (state == null || steamID == CSteamID.Nil)
                return null;

            Transform anchor;
            if (!state.DialogueAnchors.TryGetValue(
                    steamID.m_SteamID,
                    out anchor) ||
                anchor == null)
            {
                var anchorObject = new GameObject(
                    $"IntroDialogueAnchor_{steamID.m_SteamID}");
                anchor = anchorObject.transform;
                state.DialogueAnchors[steamID.m_SteamID] = anchor;
                CoopMod.Logger.LogInfo(
                    $"[IntroFormation] Created stable dialogue anchor for " +
                    NameTagManager.GetSteamDisplayName(steamID));
            }
        ...
        private void RetargetPlayerNameTag(
            CSteamID steamID,
            Transform target)
        {
            GameObject nameTag = GetPlayerNameTag(steamID);
        ...
        private void RestoreRedEyeNameTagTargets(
            RedEyeIntroFormationState state)
        {
            if (state == null)
                return;
            ...
            CSteamID localSteamID = SteamUser.GetSteamID();
            foreach (ulong steamIDValue in state.SlotPositions.Keys)
            {
                GameObject nameTag = GetPlayerNameTag(
                    new CSteamID(steamIDValue));
                nameTag?.GetComponent<PlayerNameTag>()
                    ?.SetFormationLayout(false, Vector2.zero);
            }

            foreach (ulong steamIDValue in state.DialogueAnchors.Keys)
            {
                ...
                PlayerComponent player = GetRemotePlayerComponent(steamID);
                RetargetPlayerNameTag(steamID, player?.wgo?.bubble_pos_tf);
            }
        }

        private void ApplyRedEyeFormationNameTagLayout(
            RedEyeIntroFormationState state,
            List<CSteamID> roster)
        {
            ...
            worldCenter /= playerCount;
            Camera worldCamera = MainGame.me?.world_cam ?? Camera.main;
            Vector2 screenCenter = worldCamera != null
                ? (Vector2)worldCamera.WorldToScreenPoint(worldCenter)
                : Vector2.zero;
            ...
            for (int i = 0; i < roster.Count && i < 4; i++)
            {
                ...
                ApplyRedEyeNameTagOffset(
                    GetPlayerNameTag(steamID),
                    worldCamera,
                    screenCenter,
                    slotPosition,
                    playerCount);
        ...
        private static void ApplyRedEyeNameTagOffset(
            ...
            int playerCount)
        {
            PlayerNameTag nameTag = nameTagObject != null
                ? nameTagObject.GetComponent<PlayerNameTag>()
                : null;
            if (nameTag == null)
                return;

            Vector2 direction = Vector2.zero;
            if (playerCount > 1 && worldCamera != null)
            {
                Vector2 slotScreen =
                    worldCamera.WorldToScreenPoint(slotPosition);
                direction = slotScreen - screenCenter;
            }
            // The rows already have enough world-space separation. Only move
            // labels outward horizontally so long adjacent names do not
            // overlap without lifting the rear row far above its players.
            Vector2 offset = playerCount > 1 &&
                Mathf.Abs(direction.x) > 0.01f
                    ? new Vector2(
                        Mathf.Sign(direction.x) *
                            RedEyeFormationNameTagOffset,
                        0f)
                    : Vector2.zero;
            nameTag.SetFormationLayout(true, offset);
        }

        private static Vector3 GetRedEyeFormationOffset(
            int slot,
            int rosterCount,
            Vector3 towardRedEye,
            Vector3 lateral,
            float columnSpacing,
            float rowSpacing)
        {
            int count = Mathf.Clamp(rosterCount, 1, 4);
            if (count == 1)
                return Vector3.zero;
            if (count == 2)
                return lateral * ((slot == 0 ? -0.5f : 0.5f) * columnSpacing);
            if (count == 3)
            {
                if (slot < 2)
                    return lateral * ((slot == 0 ? -0.5f : 0.5f) * columnSpacing);
                return -towardRedEye * rowSpacing;
            }

            Vector3 pairOffset = lateral *
                ((slot % 2 == 0 ? -0.5f : 0.5f) * columnSpacing);
            return slot < 2
                ? pairOffset
                : pairOffset - towardRedEye * rowSpacing;
        }

        private static void PlaceRedEyePresentationPlayer(
            PlayerComponent player,
            Vector3 position,
            GhostDriver ghost,
            WorldGameObject redEyeActor,
            Vector3 fallbackLookDirection)
        {
            ...
            if (player.wgo != null)
                player.wgo.transform.position = position;
            ghost?.ForcePosition(position);
            ...
            {
                if (redEyeActor != null)
                    character.LookAt(redEyeActor);
                else
                    character.LookAt((Vector2)fallbackLookDirection);
                if (character.anim_state != CharAnimState.Idle)
                    character.OnStopped();
            }
        }

        private static WorldGameObject ResolveRedEyeActor(
            RedEyeIntroFormationState state)
        {
            if (state.RedEyeActor != null)
                return state.RedEyeActor;

            float now = Time.realtimeSinceStartup;
            if (now < state.NextActorLookupAt)
                return null;

            state.NextActorLookupAt = now + 0.25f;
            try
            {
                state.RedEyeActor = WorldMap.GetWorldGameObjectByCustomTag(
                    "stranger",
                    true);
            }
            catch { }
        ...
        private static float CalculateRedEyeFormationSpacing()
        {
            ...
            return Mathf.Clamp(
                RedEyeFormationPixelSpacing * worldUnitsPerPixel,
                6f,
                180f);
        }

        private static List<CSteamID> GetDeterministicLobbyRoster()
        {
            var roster = new List<CSteamID>();
            SteamLobbyManager lobby = SteamLobbyManager.Instance;
            if (lobby == null || !lobby.IsInLobby ||
                lobby.CurrentLobbyID == CSteamID.Nil)
            {
                roster.Add(SteamUser.GetSteamID());
                return roster;
            }

            int memberCount = SteamMatchmaking.GetNumLobbyMembers(
            ...
            roster.Sort((left, right) =>
                left.m_SteamID.CompareTo(right.m_SteamID));
            return roster;
        }

        private static string BuildRosterSignature(List<CSteamID> roster)
        {
            ...
            for (int i = 0; i < roster.Count && i < 4; i++)
            {
                ...
                builder.Append(i + 1)
                    .Append('=')
                    .Append(NameTagManager.GetSteamDisplayName(roster[i]))
                    .Append('(')
                    .Append(roster[i].m_SteamID);
                builder.Append(')');
            }
            return builder.ToString();
        }

        private static void CopySortingLayer(
            PlayerComponent source,
            PlayerComponent destination)
        {
            if (source == null || destination == null)
                return;

            SpriteRenderer[] sourceSprites =
                source.GetComponentsInChildren<SpriteRenderer>(true);
            SpriteRenderer[] destinationSprites =
                destination.GetComponentsInChildren<SpriteRenderer>(true);
            if (sourceSprites.Length == 0 || destinationSprites.Length == 0)
                return;

            string sortingLayer = sourceSprites[0].sortingLayerName;
            for (int i = 0; i < sourceSprites.Length; i++)
            {
                SpriteRenderer renderer = sourceSprites[i];
                if (renderer != null &&
            ...
            for (int i = 0; i < destinationSprites.Length; i++)
            {
                SpriteRenderer renderer = destinationSprites[i];
                if (renderer != null &&
                    renderer.gameObject.name.IndexOf(
                        "shadow",
                        System.StringComparison.OrdinalIgnoreCase) < 0)
        ...
        {
            // The newer fishing stream owns this ghost until its explicit stop.
            // A delayed general action packet must not interrupt the fishing pose.
            if (FishingPresentationSync.Instance?.IsRemoteFishing(senderID) == true)
                return;
            if (senderID != remotePlayerSteamID)
            {
        ...
        public void ClearRemoteWorkAnimation(CSteamID peer)
        {
            if (peer == CSteamID.Nil)
                return;
            if (FishingPresentationSync.Instance?.IsRemoteFishing(peer) == true)
                return;
        ...
        {
            // Fishing owns the shared tool sprites until its presentation ends.
            if (FishingPresentationSync.Instance?.IsRemoteFishing(peer) == true)
                return;
            if (peer == remotePlayerSteamID)
            {
        ...
        public void ApplyRemoteParityAction(int animState, int itemType, int globalState, Vector2 direction, bool hasOverhead, string overheadItemJson)
        {
            ...
            try
            {
                ...
                if (FishingPresentationSync.Instance?.IsRemoteFishing(remotePlayerSteamID) == true)
                {
                    ApplyRemoteOverheadItem(character, hasOverhead, overheadItemJson);
                    return;
                }
        ...
        private static void UpdateSecondaryRemoteAnimation(SecondaryRemotePeer peer)
        {
            ...
            if (character == null)
                return;
            if (FishingPresentationSync.Instance?.IsRemoteFishing(peer.SteamID) == true)
                return;
        ...
        private static void DisableRemoteSimulation(PlayerComponent player)
        {
            ...
            // Remote presentation actors must never drive the shared footstep channel.
            if (player.Trail != null)
                player.Trail.UpdateTrails = false;
