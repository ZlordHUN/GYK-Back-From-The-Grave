...
namespace GraveyardKeeperCoop.Network
{
    ...
    public class SteamP2PManager
    {
        ...
        private struct DeferredNpcVisualPacket
        {
            public CSteamID Sender;
            public byte[] Data;
        }

        private struct PendingTimeSync
        {
            public CSteamID Sender;
            public int Day;
            public float TimeK;
        ...
        private const int MaxDeferredNpcVisualPackets = 512;
        private const int MaxDeferredNpcVisualsPerFrame = 8;
        private const int MaxDeferredNpcVisualProcessingMilliseconds = 4;
        ...
        public event Action<CSteamID, byte[]> OnFishingPresentationReceived;
        ...
        private readonly HashSet<ulong> loggedUnvalidatedPeerPackets =
            new HashSet<ulong>();
        private readonly Queue<DeferredNpcVisualPacket> deferredNpcVisualPackets =
            new Queue<DeferredNpcVisualPacket>();
        private PendingTimeSync pendingTimeSync;
        private bool hasPendingTimeSync;
        private uint nextTimeSyncSequence;
        private CSteamID lastAppliedTimeSyncSender = CSteamID.Nil;
        private uint lastAppliedTimeSyncSequence;
        private bool hasAppliedTimeSyncSequence;
        ...
        private int timeSyncsQueued;
        private int timeSyncsApplied;
        private int timeSyncsCoalesced;
        private float maxTimeSyncManagedAgeMilliseconds;
        private int npcVisualsDeferred;
        private int npcVisualsApplied;
        private int npcVisualsDropped;
        private int maxDeferredNpcVisualDepth;
        ...
        public void Update()
        {
            Multiplayer.ManualSaveIndicatorSync.Update();
            ...
            // Time is authoritative control state, whereas routine NPC visual packets
            // are lossy snapshots. Apply the newest clock before spending this frame's
            // visual budget so a visual flood cannot freeze the client's clock/lighting.
            long rawReceiveTicks = Stopwatch.GetTimestamp() - receiveStartedAt;
            long deferredReceiveStartedAt = Stopwatch.GetTimestamp();
            FlushPendingTimeSync();
            DrainDeferredNpcVisualPackets();
            long deferredReceiveTicks =
                Stopwatch.GetTimestamp() - deferredReceiveStartedAt;
            if (GraveyardKeeperCoop.Utils.FrameProfiler.Enabled)
            {
                GraveyardKeeperCoop.Utils.FrameProfiler.Record(
                    ...
                    rawReceiveTicks);
                GraveyardKeeperCoop.Utils.FrameProfiler.Record(
                    "P2P.Receive.Deferred",
                    deferredReceiveTicks);
                GraveyardKeeperCoop.Utils.FrameProfiler.Record(
                    "P2P.Receive",
                    rawReceiveTicks + deferredReceiveTicks);
            }
        ...
        public bool SendBinary(CSteamID recipientID, byte[] data, EP2PSend sendType = EP2PSend.k_EP2PSendReliable, int channel = 0)
        {
            ...
            bool isUnvalidatedHostPeer =
                ...
                !SteamLobbyManager.Instance.IsPeerCompatibilityValidated(recipientID) &&
        ...
        public bool SendCompatibilityHello(CSteamID hostID)
        {
            ...
            using (var writer = new MsgWriter(Op.Hello))
            {
                ...
                writer.Write(CoopProtocol.Revision);
        ...
        internal void DispatchReassembledMessage(CSteamID senderID, byte[] data, int lane)
        {
            ...
            if (lane == 0)
            {
                if (!SteamLobbyManager.Instance.IsPeerCompatibilityValidated(senderID))
                {
                    CoopMod.Logger.LogWarning(
                ...
                if (!TryDeferNpcVisualPacket(senderID, data, data.Length))
            ...
            if (lane != Multiplayer.LobbyChatSync.CHAT_CHANNEL ||
                ...
                !SteamLobbyManager.Instance.IsPeerCompatibilityValidated(senderID))
            {
                CoopMod.Logger.LogWarning(
        ...
        {
            Multiplayer.ManualSaveIndicatorSync.OnPeerLeftLobby(peer);
            ...
            Multiplayer.FishingPresentationSync.Instance?.OnPeerLeftLobby(peer);
            Reliable?.RemovePeer(peer);
            lastPacketRealtimeFrom.Remove(peer.m_SteamID);
            RemoveQueuedReliablePacketsFor(peer);
            RemoveDeferredNpcVisualPacketsFor(peer);
            if (hasPendingTimeSync && pendingTimeSync.Sender == peer)
                hasPendingTimeSync = false;
        ...
        {
            Multiplayer.ManualSaveIndicatorSync.Reset();
        ...
        private void HandleIncomingPacket(CSteamID senderID, byte[] data, int length)
        {
            ...
            {
                if (HandleCompatibilityHelloPacket(senderID, data, length))
                    return;
            ...
            if (TryDeferNpcVisualPacket(senderID, data, length))
                return;

            DispatchApplicationMessage(senderID, data, length);
        }

        private bool TryDeferNpcVisualPacket(
            CSteamID senderID,
            byte[] data,
            int length)
        {
            if (!BinaryProtocolExtensions.IsBinaryMessage(data, length) ||
                (Op)data[0] != Op.NpcVisualSync ||
                length > MaxUnreliablePacketBytes ||
                IsCriticalNpcVisualPacket(data, length))
            {
                return false;
            }

            // NpcVisualSync is deliberately sent as native unreliable snapshots.
            // Copy it out of Steam's reusable receive buffer and dispatch it after
            // control messages. Under sustained overload, dropping the oldest lossy
            // snapshot is safer than allowing it to block clocks, sleep, or dialogue;
            // NpcVisualSync periodically sends a full state refresh.
            if (deferredNpcVisualPackets.Count >= MaxDeferredNpcVisualPackets)
            {
                deferredNpcVisualPackets.Dequeue();
                npcVisualsDropped++;
            }

            var copy = new byte[length];
            Buffer.BlockCopy(data, 0, copy, 0, length);
            deferredNpcVisualPackets.Enqueue(new DeferredNpcVisualPacket
            {
                Sender = senderID,
                Data = copy
            });
            npcVisualsDeferred++;
            if (deferredNpcVisualPackets.Count > maxDeferredNpcVisualDepth)
                maxDeferredNpcVisualDepth = deferredNpcVisualPackets.Count;
            return true;
        }

        private static bool IsCriticalNpcVisualPacket(byte[] data, int length)
        {
            // NpcVisualSync is [op][payload length][payload][optional critical flag].
            // The tail is optional so packets from older compatible peers remain
            // readable and retain their historical lossy-snapshot semantics.
            if (data == null || length < 6 || length > data.Length)
                return false;
            ...
            int criticalFlagOffset = 5 + payloadLength;
            return criticalFlagOffset < length &&
                   data[criticalFlagOffset] != 0;
        }

        private void DrainDeferredNpcVisualPackets()
        {
            int processed = 0;
            long startedAt = Stopwatch.GetTimestamp();
            while (deferredNpcVisualPackets.Count > 0 &&
                   processed < MaxDeferredNpcVisualsPerFrame &&
                   (processed == 0 ||
                    ElapsedMilliseconds(startedAt) <
                        MaxDeferredNpcVisualProcessingMilliseconds))
            {
                DeferredNpcVisualPacket packet =
                    deferredNpcVisualPackets.Dequeue();
                if (packet.Data != null && packet.Data.Length > 0 &&
                    IsCurrentLobbyMember(packet.Sender))
                {
                    DispatchApplicationMessage(
                        packet.Sender,
                        packet.Data,
                        packet.Data.Length);
                    npcVisualsApplied++;
        ...
        private void RemoveDeferredNpcVisualPacketsFor(CSteamID peer)
        {
            int count = deferredNpcVisualPackets.Count;
            for (int i = 0; i < count; i++)
            {
                DeferredNpcVisualPacket packet =
                    deferredNpcVisualPackets.Dequeue();
                if (packet.Sender != peer)
                    deferredNpcVisualPackets.Enqueue(packet);
            }
        }

        private void ClearPriorityReceiveState()
        {
            deferredNpcVisualPackets.Clear();
            hasPendingTimeSync = false;
            nextTimeSyncSequence = 0U;
            lastAppliedTimeSyncSender = CSteamID.Nil;
            lastAppliedTimeSyncSequence = 0U;
            hasAppliedTimeSyncSequence = false;
            timeSyncsQueued = 0;
            timeSyncsApplied = 0;
            timeSyncsCoalesced = 0;
            maxTimeSyncManagedAgeMilliseconds = 0f;
            npcVisualsDeferred = 0;
            npcVisualsApplied = 0;
            npcVisualsDropped = 0;
            maxDeferredNpcVisualDepth = 0;
        }

        private static bool HandleCompatibilityHelloPacket(
            CSteamID senderID,
            byte[] data,
            int length)
        {
            ...
            try
            {
                ...
                string protocolRevision = reader.ReadString();
                if (reader.Remaining != 0 || version.Length > 64 ||
                    protocolRevision.Length > 16)
                {
                    CoopMod.Logger.LogWarning(
                        $"[P2P] Dropped malformed compatibility hello from {senderID}");
                    return true;
                }

                SteamLobbyManager.Instance.HandleCompatibilityHello(
                    senderID,
                    version,
                    protocolRevision);
            }
        ...
        private void MaybeReportReceiveTelemetry()
        {
                ...
                npcVisualsDropped > 0)
            {
                ...
                CoopMod.Logger.LogWarning(
                    ...
                    $"time_priority={timeSyncsQueued}/{timeSyncsApplied}/" +
                    $"{timeSyncsCoalesced}, time_managed_age_max=" +
                    $"{maxTimeSyncManagedAgeMilliseconds:F1}ms, " +
                    $"deferred_npc_visual={npcVisualsDeferred}/{npcVisualsApplied}/" +
                    $"{npcVisualsDropped}, deferred_depth_max=" +
                    $"{maxDeferredNpcVisualDepth}, " +
                    $"top_app=[{details}]");
            }
        ...
        private static bool IsKnownNativeUnreliablePacket(byte[] data, int length)
        {
            ...
            // Steam does not expose the original send mode on receive. The shared,
            // tested policy admits lossy state streams even after RNET negotiation;
            // reliable-only gameplay commands still cannot bypass that channel.
            Op op = (Op)data[0];
            bool isBuildingPreview = op == Op.SpawnSync &&
                Multiplayer.SpawnSync.IsBuildingPreviewWirePacket(data, length);
            return NativePacketPolicy.IsUnreliable(op, isBuildingPreview);
        }
        ...
        private void HandleBinaryMessage(CSteamID senderID, byte[] data, int length)
        {
            ...
            switch (reader.Op)
            {
                ...
                case Op.FishingPresentation:
                    HandleSyncPayload(senderID, ref reader, OnFishingPresentationReceived);
                    break;
                ...
                case Op.ManualSaveActivity:
                    Multiplayer.ManualSaveIndicatorSync.HandleRemote(senderID, ref reader);
                    break;

                case Op.DebugGiveRequest:
                    Multiplayer.DebugChatCommands.HandleGiveRequestMessage(
                        senderID,
                        ref reader);
                    break;

                case Op.DebugGiveGrant:
                    Multiplayer.DebugChatCommands.HandleGiveGrantMessage(
                        senderID,
                        ref reader);
                    break;

                case Op.DebugGiveResult:
                    Multiplayer.DebugChatCommands.HandleGiveResultMessage(
                        senderID,
                        ref reader);
                    break;
        ...
        private void HandleInviteMessage(CSteamID senderID, ref MsgReader reader)
        {
            try
            {
                ...
                string protocolRevision = reader.Remaining > 0 ? reader.ReadString() : "";
                ...
                {
                    CoopMod.Logger.LogWarning(
                        $"[P2P] Invite rejected by compatibility check: " +
                        compatibilityRejectMessage.Replace('\n', ' '));
                    ShowIncompatibleInviteNotification(
                        inviterName,
                        modVersion,
                        protocolRevision);
                    return;
        ...
        private void ShowIncompatibleInviteNotification(
            string inviterName,
            string hostVersion,
            string hostProtocolRevision)
        {
            string message = SteamLobbyManager.BuildInviteCompatibilityNotice(
                inviterName,
                hostVersion,
            ...
            if (UI.JoinGameGUI.Instance != null && UI.JoinGameGUI.Instance.is_shown)
            {
                UI.JoinGameGUI.Instance.ShowIncompatibleInvitation(
                    inviterName,
                    message);
                return;
            }
            ...
            if (UI.JoinGameGUI.Instance != null)
            {
                ...
                if (runner != null)
                {
                    runner.StartCoroutine(
                        ShowIncompatibleInvitationAfterDelay(inviterName, message));
                    return;
                }
            }
        ...
        private System.Collections.IEnumerator ShowIncompatibleInvitationAfterDelay(
            string inviterName,
            string message)
        {
        ...
        private void HandleLobbyInfo(CSteamID senderID, ref MsgReader reader)
        {
            ...
            try
            {
                ...
                {
                    CoopMod.Logger.LogWarning(
                        $"[P2P] Lobby info rejected by compatibility check: " +
                        compatibilityRejectMessage.Replace('\n', ' '));
                    GUIElements.me?.dialog?.OpenOK(compatibilityRejectMessage);
                    return;
                }
        ...
        /// </summary>
        public uint BroadcastTimeSync(int day, float timeK)
        {
            using (var w = new MsgWriter(Op.TimeSync, 16))
            {
                w.Write(day);
                w.Write(timeK);
                nextTimeSyncSequence++;
                if (nextTimeSyncSequence == 0U)
                    nextTimeSyncSequence = 1U;
                // Optional freshness tail. Older peers read the original day/time
                // prefix and ignore it; newer peers use it to reject reordered
                // unreliable clock snapshots without a protocol revision bump.
                w.Write(nextTimeSyncSequence);
                BroadcastBinary(w.ToArray(), EP2PSend.k_EP2PSendUnreliable);
            }
            return nextTimeSyncSequence;
        }

        /// <summary>
        /// Records a clock sequence that another host-authenticated control message
        /// already applied. WakeAll uses this as a reliable freshness fence so an
        /// older unreliable TimeSync cannot roll the exact wake/autosave clock back.
        /// </summary>
        public void RecordAppliedTimeSyncSequence(
            CSteamID senderID,
            uint sequence)
        {
            CSteamID lobby =
                SteamLobbyManager.Instance?.CurrentLobbyID ?? CSteamID.Nil;
            if (sequence == 0U || lobby == CSteamID.Nil ||
                SteamMatchmaking.GetLobbyOwner(lobby) != senderID)
            {
                return;
            }

            if (hasPendingTimeSync &&
                pendingTimeSync.Sender == senderID &&
                pendingTimeSync.HasSequence &&
                !IsNewerSequence(pendingTimeSync.Sequence, sequence))
            {
                hasPendingTimeSync = false;
                timeSyncsCoalesced++;
            }

            if (!hasAppliedTimeSyncSequence ||
                lastAppliedTimeSyncSender != senderID ||
                IsNewerSequence(sequence, lastAppliedTimeSyncSequence))
            {
                lastAppliedTimeSyncSender = senderID;
                lastAppliedTimeSyncSequence = sequence;
                hasAppliedTimeSyncSequence = true;
            }
        }
        ...
        private void HandleTimeSyncMessage(CSteamID senderID, ref MsgReader reader)
        {
            CSteamID lobby =
                SteamLobbyManager.Instance?.CurrentLobbyID ?? CSteamID.Nil;
            if (reader.Remaining < 8 || lobby == CSteamID.Nil ||
                SteamMatchmaking.GetLobbyOwner(lobby) != senderID)
            {
                return;
            }
            ...
            if (day < 0 || float.IsNaN(timeK) || float.IsInfinity(timeK))
                return;

            bool hasSequence = reader.Remaining >= 4;
            uint sequence = hasSequence ? reader.ReadUInt32() : 0U;
            if (hasSequence && hasPendingTimeSync &&
                pendingTimeSync.HasSequence &&
                pendingTimeSync.Sender == senderID &&
                !IsNewerSequence(sequence, pendingTimeSync.Sequence))
            {
                timeSyncsCoalesced++;
                return;
            }
            if (hasSequence && !hasPendingTimeSync &&
                hasAppliedTimeSyncSequence &&
                lastAppliedTimeSyncSender == senderID &&
                !IsNewerSequence(sequence, lastAppliedTimeSyncSequence))
            {
                timeSyncsCoalesced++;
            ...
            pendingTimeSync = new PendingTimeSync
            {
                Sender = senderID,
                Day = day,
                TimeK = timeK,
            ...
            hasPendingTimeSync = true;
            timeSyncsQueued++;
        }

        private void FlushPendingTimeSync()
        {
            if (!hasPendingTimeSync)
                return;

            PendingTimeSync latest = pendingTimeSync;
            hasPendingTimeSync = false;
            float ageMilliseconds = Mathf.Max(
                0f,
                (Time.realtimeSinceStartup - latest.QueuedAt) * 1000f);
            if (ageMilliseconds > maxTimeSyncManagedAgeMilliseconds)
                maxTimeSyncManagedAgeMilliseconds = ageMilliseconds;

            OnTimeSyncReceived?.Invoke(
                latest.Sender,
                latest.Day,
                latest.TimeK);
            if (latest.HasSequence)
            {
                lastAppliedTimeSyncSender = latest.Sender;
                lastAppliedTimeSyncSequence = latest.Sequence;
                hasAppliedTimeSyncSequence = true;
            }
            timeSyncsApplied++;
        }
        ...
        public void BroadcastFishingPresentation(byte[] payload, bool reliable)
        {
            using (var writer = new MsgWriter(Op.FishingPresentation, (payload?.Length ?? 0) + 8))
            {
                writer.WriteBytes(payload ?? new byte[0]);
                BroadcastBinary(writer.ToArray(), reliable
            ...
            bool reliable = false)
        {
            ...
            {
                w.WriteBytes(payload ?? new byte[0]);
                // Reliable state repairs (for example Bishop placement) must never
                // enter the bounded lossy visual queue on either native or RNET
                // receive paths. Older readers safely ignore this optional tail.
                w.Write(reliable);
                BroadcastBinary(
                    w.ToArray(),
                    reliable
        ...
        /// <summary>
        /// Client -> host: request an item grant for a current session player.
        /// The host authenticates the sender and revalidates cheats, target, item,
        /// and amount before applying or forwarding the request.
        /// </summary>
        public bool SendDebugGiveRequest(
            CSteamID target,
            string itemId,
            int amount)
        {
            ...
            if (lobby == null || !lobby.IsInLobby || lobby.IsHost ||
                target == CSteamID.Nil || target == SteamUser.GetSteamID() ||
                !IsCurrentLobbyMember(target) ||
                string.IsNullOrWhiteSpace(itemId) || itemId.Length > 128 ||
                amount < 1 || amount > 999)
            {
                return false;
            }

            using (var writer = new MsgWriter(Op.DebugGiveRequest, 160))
            {
                ...
                bool sent = SendToHost(
                    ...
                    EP2PSend.k_EP2PSendReliable);
                CoopMod.Logger.LogInfo(
                    $"[P2P] Sent DebugGiveRequest to host: target={target}, " +
                    $"item='{itemId}', amount={amount}, sent={sent}");
                return sent;
            }
        }

        /// <summary>
        /// Host -> target client: tell the peer that owns the target player to add
        /// the item to its local MainGame player inventory.
        /// </summary>
        public bool SendDebugGiveGrant(
            CSteamID target,
            string itemId,
            int amount)
        {
            ...
            using (var writer = new MsgWriter(Op.DebugGiveGrant, 152))
            {
                writer.Write(amount);
                writer.Write(itemId);
                bool sent = SendBinary(
                    target,
                    writer.ToArray(),
                    EP2PSend.k_EP2PSendReliable);
                CoopMod.Logger.LogInfo(
                    $"[P2P] Sent DebugGiveGrant to {target}: item='{itemId}', " +
        ...
        /// <summary>
        /// Host -> requesting client: surface whether the host accepted or rejected
        /// the request. Delivery means the grant was queued reliably; the target
        /// still decides whether its local inventory can accept the item.
        /// </summary>
        public bool SendDebugGiveResult(
            CSteamID target,
            bool accepted,
            string message)
        {
            ...
            if (lobby == null || !lobby.IsInLobby || !lobby.IsHost ||
                target == CSteamID.Nil || target == SteamUser.GetSteamID() ||
                !IsCurrentLobbyMember(target) ||
                string.IsNullOrWhiteSpace(message) || message.Length > 512)
            {
                return false;
            }

            using (var writer = new MsgWriter(Op.DebugGiveResult, 256))
            {
