...
namespace GraveyardKeeperCoop.Patches
{
    ...
    public class OnlineDialoguePatches
    {
        ...
        private static CSteamID pendingSpeechBubbleNameTagOwner = CSteamID.Nil;
        ...
        {
            pendingSpeechBubbleNameTagOwner = CSteamID.Nil;
            var onlineCoop = OnlineCoopManager.Instance;
            if (onlineCoop == null || !onlineCoop.IsOnlineCoopEnabled)
                return;
            ...
            CSteamID ownerSteamID = dialogueSync.LocalPlayerLastAdvanced
                ? SteamUser.GetSteamID()
                : dialogueSync.LastDialogueAdvancer;
            bool usedFormationAnchor = onlineCoop.TryGetRedEyeIntroDialogueAnchor(
                ownerSteamID,
                out Transform dialogueAnchor);
            if (!usedFormationAnchor)
                dialogueAnchor = owner?.bubble_pos_tf;
            if (owner != null && dialogueAnchor != null)
            {
                speaker_id = owner.unique_id;
                link = dialogueAnchor;
                pendingSpeechBubbleNameTagOwner = ownerSteamID;
                CoopMod.Logger.LogInfo(
                    $"[OnlineDialogue] Routed speech bubble to " +
                    $"{(dialogueSync.LocalPlayerLastAdvanced ? "local" : "remote")} " +
                    $"dialogue advancer{(usedFormationAnchor ? " via intro formation anchor" : "")}");
            }
        }
        ...
        {
            CSteamID nameTagOwner = pendingSpeechBubbleNameTagOwner;
            pendingSpeechBubbleNameTagOwner = CSteamID.Nil;
                ...
                bubble != null)
            {
                ...
                if (nameTagOwner != CSteamID.Nil)
                {
                    OnlineCoopManager.Instance?.SuppressPlayerNameTagForBubble(
                        nameTagOwner,
                        bubble.GetInstanceID());
                }

                NameTagManager.EnsureBelowSpeechBubble(bubble);
        ...
        public static void SpeechBubbleGUI_DestroyBubble_Postfix(SpeechBubbleGUI __instance)
        {
            if (__instance != null)
            {
                OnlineCoopManager.Instance?.RestorePlayerNameTagForBubble(
                    __instance.GetInstanceID());
            }
            NameTagManager.RestoreDefaultRenderDepthIfNoSpeechBubbles();
            ...
            bool show_to_left)
        {
            ...
            CSteamID ownerSteamID = dialogueSync.LocalPlayerLastAdvanced
                ? SteamUser.GetSteamID()
                : dialogueSync.LastDialogueAdvancer;
            if (!onlineCoop.TryGetRedEyeIntroDialogueAnchor(
                    ownerSteamID,
                    out Transform dialogueAnchor))
            {
                dialogueAnchor = owner?.bubble_pos_tf;
            ...
            bool showToLeft)
        {
            ...
            Transform remoteDialogueAnchor = null;
            OnlineCoopManager.Instance?.TryGetRedEyeIntroDialogueAnchor(
                senderID,
                out remoteDialogueAnchor);
            if (remoteDialogueAnchor == null)
                remoteDialogueAnchor = remotePlayer?.bubble_pos_tf;
            if (remotePlayer == null || remoteDialogueAnchor == null)
            {
                CoopMod.Logger.LogWarning(
