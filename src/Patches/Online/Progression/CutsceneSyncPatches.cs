...
namespace GraveyardKeeperCoop.Patches
{
    ...
    public static class CutsceneSyncPatches
    {
        ...
        private const string YORICK_FIRST_SLEEP_SCRIPT =
            "ghost_comes_after_players_sleep";
        private const string YORICK_PENDING_PLAYER_PARAM =
            "ghost_comes_2_home_after_1st_burial";
        private const string TALKING_SKULL_TAG = "talking_skull";
        private const string GERRY_RIVER_PENDING_PARAM =
            "on_showing_gerry_near_river";
            ...
            ref CustomFlowScript __result)
        {
            ...
            if (GameTimeSync.Instance?
                    .TryConsumeSynchronizedFirstSleepStorySuppression(
                        uscript_name) == true)
            {
                __result = null;
                CoopMod.Logger.LogInfo(
                    "[CutsceneSync] Suppressed duplicate Yorick FlowScript " +
                    "during synchronized non-host wake");
                return false;
            }

            var onlineCoop = OnlineCoopManager.Instance;
            if (StoryCatalogRuntime.RequiresLobbyHostExecution(uscript_name) &&
                onlineCoop?.IsOnlineCoopEnabled == true &&
                !onlineCoop.IsHost)
            {
                // Flow_RunScript always invokes its immediate Out port after this
                // call. Returning null suppresses only the child cutscene; the
                // surrounding sleep graph can still finish normally on the client.
                __result = null;
                CoopMod.Logger.LogInfo(
                    $"[CutsceneSync] Suppressed lobby-host-owned FlowScript " +
                    $"\"{uscript_name}\" on non-host peer");
                return false;
            }

            if (StoryCatalogRuntime.UsesFormationPolicy(
                    uscript_name,
                    "intro_red_eye_party"))
            {
                OnlineCoopManager.Instance?.BeginRedEyeIntroFormation(
                    uscript_name);
            }

            if (isProcessingRemote)
            {
                StoryCatalogRuntime.ObserveFlowScriptStarted(
                    uscript_name,
                    "global",
                    remote: true);
                return true;
        ...
        private static void PromotePotentialCinematicIfNeeded()
        {
            ...
            // Player-bound cutscenes belong to the player who triggered them. The
            // catalog marks reciprocal global mirrors that require a deterministic
            // lobby-host owner so both peers cannot claim the first player line.
            EnsureSharedCutsceneDialogueOwner(
                StoryCatalogRuntime.RequiresLobbyHostDialogueOwner(candidate.ScriptName)
                    ? CSteamID.Nil
        ...
        private static void OnCutsceneSyncReceived(CSteamID senderID, string scriptName, Vector3 originWorldPos, byte facing, string originZoneId, bool hasScope)
        {
            ...
            if (StoryCatalogRuntime.RequiresLobbyHostExecution(scriptName) &&
                !IsLobbyHostSender(senderID))
            {
                CoopMod.Logger.LogWarning(
                    $"[CutsceneSync] Ignoring lobby-host-owned FlowScript " +
                    $"trigger \"{scriptName}\" from non-host peer {senderID}");
                return;
            }
            if (StoryCatalogRuntime.TryGetParticipantFacing(
                    scriptName,
                    out Direction participantFacing))
            {
                facing = (byte)participantFacing;
            }
        ...
        private static void ApplyCataloguedParticipantFacing(
            string scriptName,
            string reason)
        {
            if (!StoryCatalogRuntime.TryGetParticipantFacing(
            ...
            try
            {
                BaseCharacterComponent player = MainGame.me?.player_char;
                if (player == null)
                    return;

                player.StopMovement();
                player.LookAt(facing);
                CoopMod.Logger.LogInfo(
                    $"[CutsceneSync] Applied catalogued participant facing " +
                    $"{facing} for \"{scriptName}\" ({reason})");
            }
            catch (System.Exception ex)
            {
                CoopMod.Logger.LogWarning(
                    $"[CutsceneSync] Failed to apply participant facing for " +
        ...
        private static void OnCutsceneCompleteReceived(CSteamID senderID, string scriptName, Vector3 originWorldPos, string originZoneId)
        {
                ...
                !IsLobbyHostSender(senderID))
            {
                CoopMod.Logger.LogWarning(
                    $"[CutsceneSync] Ignoring lobby-host-owned FlowScript " +
                    $"completion \"{scriptName}\" from non-host peer {senderID}");
                return;
            }
                ...
                !session.MirrorsLocalExecution)
            {
                ...
                UnlockLocalPlayerAfterCutscene();
                ApplyCataloguedParticipantFacing(
                    scriptName,
                    "remote cutscene completed");
            }

            ApplyRemoteYorickCompletionState(scriptName);
            QuestSideEffectSync.ApplyRemoteCutscenePersonalItemGrants(
                scriptName);
        ...
        private static void ApplyRemoteYorickCompletionState(
            string scriptName)
        {
            if (!string.Equals(
                    scriptName,
                    YORICK_FIRST_SLEEP_SCRIPT,
                    System.StringComparison.OrdinalIgnoreCase))
            {
                return;
            }
            ...
            if (player != null)
            {
                player.SetParam(YORICK_PENDING_PLAYER_PARAM, 0f);
            }

            WorldGameObject talkingSkull = null;
            try
            {
                talkingSkull = WorldMap.GetWorldGameObjectByCustomTag(
                    TALKING_SKULL_TAG,
                    false);
            }
            catch { }
            talkingSkull?.SetParam(GERRY_RIVER_PENDING_PARAM, 1f);

            OnlineCoopManager onlineCoop = OnlineCoopManager.Instance;
            if (onlineCoop?.IsHost == true)
            {
                PlayerParamSync.Instance?.SendLocalSnapshot(force: true);
                WGOStateSync.Instance?.SendLocalSnapshot(force: false);
            }

            CoopMod.Logger.LogInfo(
                "[CutsceneSync] Applied canonical Yorick completion state " +
                "before allocating observer rewards");
        }
            ...
            string reason)
        {
            ...
            if (!localCutscene.SuppressCompletionBroadcast)
            {
                ApplyCataloguedParticipantFacing(
                    ...
                    "authoritative cutscene completed");
                // Vanilla normally auto-collects before control returns. Confirm
                // that copy, or retry one missing allocation, before receipting it.
                QuestSideEffectSync.EnsureLocalCutscenePersonalItemGrantAllocation(
                    localCutscene.ScriptName);
        ...
        private static bool TryJoinRemoteCutsceneSession(string reason, bool requireProximity)
        {
            ...
            if (HasActiveLocalSyncedCutscene(session.ScriptName))
            {
                ...
                EnsureSharedCutsceneDialogueOwner(
                    StoryCatalogRuntime.RequiresLobbyHostDialogueOwner(session.ScriptName)
                        ? CSteamID.Nil
                        : session.SenderID);
                CoopMod.Logger.LogInfo(
            ...
            // only scripts whose catalog policy requires local execution while
            // suppressing their completion echo. Player-bound interactions remain
            // owned by the initiator.
            if (StoryCatalogRuntime.RequiresLocalFlowScriptMirror(session.ScriptName) &&
            ...
            if (StoryCatalogRuntime.TryGetParticipantFacing(
                    ...
                    out Direction participantFacing))
            {
                session.Facing = (byte)participantFacing;
            }
            else try
            {
        ...
        internal static bool IsCutsceneSessionActive(string scriptName)
        {
            if (string.IsNullOrEmpty(scriptName))
                return false;
        ...
        private static bool IsLobbyHostSender(CSteamID senderID)
        {
            CSteamID owner =
                SteamLobbyManager.Instance?.GetLobbyOwner() ?? CSteamID.Nil;
            return owner != CSteamID.Nil && senderID == owner;
        }
