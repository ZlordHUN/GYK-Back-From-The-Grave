...
namespace GraveyardKeeperCoop.Patches
{
    ...
    [HarmonyPatch]
    internal static class ZoneNavLookupInvalidationPatches
    {
        private static void Invalidate()
        {
            GraveyardKeeperCoop.Multiplayer.ZoneNavSync.Instance
                ?.InvalidateGDPointIndex();
        }

        [HarmonyPatch(typeof(WorldMap), nameof(WorldMap.RescanGDPoints))]
        [HarmonyPostfix]
        private static void RescanGDPoints_Postfix() => Invalidate();

        [HarmonyPatch(typeof(WorldMap), nameof(WorldMap.ImportGDPointsOnLoadedScene))]
        [HarmonyPostfix]
        private static void ImportGDPoints_Postfix() => Invalidate();

        [HarmonyPatch(typeof(WorldMap), nameof(WorldMap.ExportGDPointsOnUnloadedScene))]
        [HarmonyPostfix]
        private static void ExportGDPoints_Postfix() => Invalidate();
    }
