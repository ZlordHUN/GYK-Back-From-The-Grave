...
namespace GraveyardKeeperCoop.Patches
{
    ...
    public class MapGUIPatches
    {
        ...
        private static GameObject topMovementIndicator;
        private static long nextMovementOrder;
        private static bool indicatorDepthsDirty;
        ...
        [HarmonyPostfix]
        [HarmonyPatch("Open")]
        public static void Open_Postfix(MapGUI __instance, ref Vector2 ____momentum)
        {
            // MapGUI keeps its controller momentum when disabled. Clear it before
            // recentering so the previous inspection direction cannot move the map
            // again on the first frame after it opens.
            ____momentum = Vector2.zero;
            ...
            try
            {
                // Opening the map should begin at this peer's player, not at the last
                // location they inspected. This is intentionally open-only: subsequent
                // dragging remains untouched until the map is closed and opened again.
                CenterMapOnLocalPlayer(__instance);
            }
            catch (System.Exception ex)
            {
                CoopMod.Logger.LogError($"Error centering map on local player: {ex.Message}");
            }
        }

        private static void CenterMapOnLocalPlayer(MapGUI mapGUI)
        {
            if (mapGUI == null || MainGame.me?.player == null)
                return;
            ...
            // Anchors are normally refreshed later in the first enabled frame. Force
            // that refresh before resolving either target or viewport coordinates so
            // a resolution/UI-scale change cannot leave either one stale.
            panel.UpdateAnchors();

            // Resolve the position independently of the optional indicator visual so
            // an avatar/template failure cannot prevent the map itself from centering.
            if (scrollView.transform.childCount == 0)
                return;

            Transform contents = scrollView.transform.GetChild(0);
            Vector2 mapPosition = MapCoordinateHelper.PlayerWorldToMap(
                MainGame.me.player.transform.position);
            Vector3 targetWorldPosition = contents.TransformPoint(
                new Vector3(mapPosition.x, mapPosition.y, 0f));

            // UIScrollView retains mouse-wheel/drag momentum and an active spring while
            // disabled. Stop both without permanently changing its configured inertia.
            scrollView.DisableSpring();
            float momentumAmount = scrollView.momentumAmount;
            scrollView.StopScrolling();
            scrollView.momentumAmount = momentumAmount;

            Vector3[] worldCorners = panel.worldCorners;
            if (worldCorners == null || worldCorners.Length < 3)
                return;

            Transform panelTransform = panel.cachedTransform;
            Vector3 panelCenter = (worldCorners[0] + worldCorners[2]) * 0.5f;
            Vector3 targetInPanel = panelTransform.InverseTransformPoint(targetWorldPosition);
            Vector3 centerInPanel = panelTransform.InverseTransformPoint(panelCenter);
            Vector3 movement = centerInPanel - targetInPanel;
            ...
            scrollView.RestrictWithinBounds(
                true,
                scrollView.canMoveHorizontally,
        ...
        /// <summary>
        /// Reapply vanilla map-zone visibility after ZoneNavSync learns about a
        /// zone while the map is already open.
        /// </summary>
        public static void RefreshKnownZoneVisibility()
        {
            try
            {
                var mapGUI = cachedMapGUI;
                var save = MainGame.me?.save;
                if (mapGUI == null || save?.known_world_zones == null ||
                    !mapGUI.gameObject.activeInHierarchy)
                {
                    return;
                }

                MapZoneGUI[] zones = mapGUI.GetComponentsInChildren<MapZoneGUI>(true);
                for (int i = 0; i < zones.Length; i++)
                {
                    MapZoneGUI zone = zones[i];
                    if (zone == null) continue;

                    bool known = save.known_world_zones.Contains(zone.name);
                    if (zone.label != null)
                    {
                        zone.label.text = GJL.L(string.IsNullOrEmpty(zone.override_name)
                            ? "zone_" + zone.name
                            : zone.override_name);
                    }
                    zone.gameObject.SetActive(known);
                    if (zone.hidden != null)
                    {
                        zone.hidden.SetActive(!known);
                    }
                }
            }
            catch (System.Exception ex)
            {
                CoopMod.Logger.LogError($"Error refreshing live map-zone visibility: {ex.Message}");
            }
        }
        ...
        private static void UpdateLocalPlayerIndicator(MapGUI mapGUI, bool logDetails = false)
        {
            ...
            ApplyIndicatorPosition(
                ...
                playerWorldPos,
                new Vector3(mapPos.x, mapPos.y, 0f));
            ...
            float pulseScale)
        {
            ...
            if (frameTexture != null)
            {
                ...
                frameWidget = frameObject.AddComponent<UITexture>();
            ...
            Color indicatorColor)
        {
            ...
            ApplyIndicatorPosition(indicator, worldPosition, nextMapPosition);
        ...
        private static void ApplyIndicatorPosition(
            ...
            Vector3 mapPosition)
        {
            if (indicator == null)
                return;
            if (!indicatorVisualStates.TryGetValue(indicator, out IndicatorVisualState state))
            {
                indicator.transform.localPosition = mapPosition;
                return;
            }

            bool mapMoved = !state.HasMapPosition || state.LastMapPosition != mapPosition;
            if (mapMoved)
            {
                indicator.transform.localPosition = mapPosition;
                state.LastMapPosition = mapPosition;
                state.HasMapPosition = true;

                // NGUI can retain previous geometry while the game is paused.
                MarkIndicatorChanged(state);
            }

            bool playerMoved = !state.HasWorldPosition ||
                               (state.LastWorldPosition - worldPosition).sqrMagnitude > 0.0001f;
            if (!playerMoved)
                return;

            state.LastWorldPosition = worldPosition;
            state.HasWorldPosition = true;
            state.LastMovementOrder = ++nextMovementOrder;
            if (topMovementIndicator != indicator)
            {
                topMovementIndicator = indicator;
                indicatorDepthsDirty = true;
            }
        }

        private static void FlushIndicatorDepths()
        {
            if (!indicatorDepthsDirty)
                return;

            indicatorDepthsDirty = false;
            RefreshIndicatorDepths();
        }

        private static void RefreshIndicatorDepths()
        {
            foreach (KeyValuePair<GameObject, IndicatorVisualState> pair in indicatorVisualStates)
            {
                GameObject indicator = pair.Key;
                IndicatorVisualState state = pair.Value;
                bool onTop = indicator != null && indicator == topMovementIndicator;

                if (state.AvatarWidget != null)
                    state.AvatarWidget.depth = AvatarDepth + (onTop ? 2 : 0);
                if (state.FrameWidget != null)
                    state.FrameWidget.depth = FrameDepth + (onTop ? 2 : 0);
                MarkIndicatorChanged(state);
            }
        }

        private static void MarkIndicatorChanged(IndicatorVisualState state)
        {
            UIWidget[] widgets = state?.Widgets;
            if (widgets == null)
                return;
        ...
        private static void DestroyIndicator(GameObject indicator)
        {
            ...
            bool removedTop = topMovementIndicator == indicator;
            indicatorVisualStates.Remove(indicator);
            if (removedTop)
            {
                topMovementIndicator = FindMostRecentlyMovedIndicator();
                indicatorDepthsDirty = true;
                FlushIndicatorDepths();
            }
        ...
        private static GameObject FindMostRecentlyMovedIndicator()
        {
            GameObject result = null;
            long latestOrder = long.MinValue;
            foreach (KeyValuePair<GameObject, IndicatorVisualState> pair in indicatorVisualStates)
            {
                if (pair.Key == null || pair.Value.LastMovementOrder <= latestOrder)
                    continue;

                result = pair.Key;
                latestOrder = pair.Value.LastMovementOrder;
            }
            return result;
        }
    }
