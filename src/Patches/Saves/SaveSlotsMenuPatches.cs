...
namespace GraveyardKeeperCoop.Patches
{
    ...
    public class SaveSlotsMenuPatches
    {
        ...
        /// <summary>
        /// Clear the transient save/load menu state after a completed manual save.
        /// Leaving IsSaveGameMode set would cause a later automatic save to inherit
        /// manual-save naming behavior.
        /// </summary>
        private static void ResetSaveLoadModeAfterSave()
        {
        ...
        [HarmonyPrefix]
        public static bool OnSlotsLoaded_Prefix(SaveSlotsMenuGUI __instance, ref List<SaveSlotData> slots)
        {
            ...
            if (ManualSaveSlotsGUI.Instance != null && ManualSaveSlotsGUI.Instance.IsInCustomMode())
            {
                ...
                // Replace only the callback argument. ReadSaveSlots also stores this list
                // internally for filename allocation, so mutating it can make a hidden
                // save's filename look available and cause the next save to overwrite it.
                List<SaveSlotData> customSlots = ManualSaveSlotsGUI.Instance.GetCustomSlots();
                slots = customSlots != null
                    ? new List<SaveSlotData>(customSlots)
                    : new List<SaveSlotData>();
                return true;
            }

            // Filter a copy based on multiplayer context. PlatformSpecific owns the
            // original list and its GetNewSlotFilename implementation relies on every
            // occupied filename remaining in that list, including hidden saves.
            List<SaveSlotData> sourceSlots = slots ?? new List<SaveSlotData>();
            bool useMultiplayerSaves = MainMenuPatches.IsMultiplayerSaveMode ||
                                       MainMenuPatches.IsMultiplayerSaveContextActive();
            if (useMultiplayerSaves)
            {
                // In multiplayer mode: only show coop saves
                slots = sourceSlots.Where(MainMenuPatches.IsMultiplayerSave).ToList();
                CoopMod.Logger.LogInfo($"[SaveSlotsMenuPatch] Multiplayer mode - showing {slots.Count} of {sourceSlots.Count} saves");
            }
            else
            {
                // In single-player mode: hide coop saves
                slots = sourceSlots.Where(s => !MainMenuPatches.IsMultiplayerSave(s)).ToList();
                CoopMod.Logger.LogInfo($"[SaveSlotsMenuPatch] Single-player mode - showing {slots.Count} of {sourceSlots.Count} saves (coop saves hidden)");
            }
            ...
            if (MainMenuPatches.IsSaveGameMode)
            {
                if (slot == null)
                {
                    ...
                    GraveyardKeeperCoop.UI.RenameSaveDialog.Show(
                        ...
                        onConfirm: customName =>
                        {
                            ...
                            bool saveCompletionPending = true;
                            ManualSaveIndicatorSync.BeginLocal();

                            try
                            {
                                // Capture current position, inventory, and player state
                                // after the user confirms the save and while the shared
                                // vanilla saving indicator is already visible.
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        // Update the linked save to the current game state instead of
                        // writing the state captured when this slot was first created.
