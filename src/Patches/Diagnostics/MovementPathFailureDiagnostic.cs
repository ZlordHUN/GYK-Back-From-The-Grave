...
namespace GraveyardKeeperCoop.Patches
{
    ...
    public static class MovementPathFailureDiagnostic
    {
        ...
        private const float MaxBishopDepartureRecoveryDistance = 4096f;
        ...
        public static bool Prefix(MovementComponent __instance)
        {
            try
            {
                ...
                bool isAuthoritativeBishopRepair =
                    !isDonkey &&
                    onlineCoop != null &&
                    onlineCoop.IsOnlineCoopEnabled &&
                    onlineCoop.IsHost &&
                    dist <= MaxBishopDepartureRecoveryDistance &&
                    MovementOnCompleteField?.GetValue(__instance) != null &&
                    BishopScheduleRepairPatches.IsRepairDepartureRoute(
                        __instance,
                        wgo,
                        dest);
                if (isAuthoritativeBishopRepair &&
                    TryBeginCutsceneRecovery(__instance, wgo, dest))
                {
                    CoopMod.Logger.LogWarning(
                        "[PathFail] Replaced stalled repair-owned Bishop " +
                        $"departure with bounded direct recovery to {dest}");
                    return false;
                }
