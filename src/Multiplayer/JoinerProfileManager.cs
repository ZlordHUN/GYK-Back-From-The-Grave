...
namespace GraveyardKeeperCoop.Multiplayer
{
    ...
    /// health, energy state, technology-point and refugee-gratitude balances, and
    /// active effects across host-save loads.
    ...
    public class JoinerProfileManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            CoopMod.Logger.LogInfo(
                ...
                "(personal inventory/loadout/money/carried item/health/energy/technology points/refugee gratitude/effects)");
        }
            ...
            string saveHash)
        {
            ...
            {
                CoopMod.Logger.LogInfo(
                    $"[JoinerProfile] Applied personal inventory/loadout/money/carried item/health/energy/technology points/refugee gratitude/effects " +
                    $"(inventory={profile.InventoryCount}, toolbelt={profile.ToolbeltCount}, " +
                    $"money={profile.Money:F2}, overhead={profile.OverheadItemId}, " +
                    $"hp={profile.Health:F2}, energy={profile.Energy:F2}, " +
                    $"tech_points={profile.TechnologyPointsSummary}, " +
        ...
        private sealed class JoinerProfile
        {
            ...
            private struct SharedStoryToolSnapshot
            {
                public string ItemJson;
                public bool InToolbelt;
                public int Index;

                public bool IsPresent =>
                    !string.IsNullOrEmpty(ItemJson);
            }
            ...
            public bool ApplyToLivePlayer()
            {
                ...
                // Story tools follow shared host-world progression, not a joiner's
                // profile history. Capture the shovel state from the downloaded
                // host save before replacing personal inventory so a profile from
                // a later revision cannot inject it before Gerry grants it.
                SharedStoryToolSnapshot hostShovel =
                    CaptureSharedStoryTool(
                        data,
                ...
                SharedStoryToolSnapshot profileShovel =
                    CaptureSharedStoryTool(
                        data,
                        SharedCemeteryShovelId);
                int removedProfileShovels =
                    RemoveItemsById(
                        data.inventory,
                        SharedCemeteryShovelId,
                        0) +
                    RemoveItemsById(
                        data.secondary_inventory,
                        SharedCemeteryShovelId,
                        0);
                if (hostShovel.IsPresent)
                {
                    // Once the shared story grant exists, retain this player's
                    // own shovel durability/slot. A first-time joiner receives
                    // the host-save copy as the baseline.
                    RestoreSharedStoryTool(
                        data,
                        profileShovel.IsPresent
                            ? profileShovel
                            : hostShovel);
                }
                else
                {
                    ClearEquippedItem(save, SharedCemeteryShovelId);
                }
                ...
                if (removedProfileShovels > 0 ||
                    hostShovel.IsPresent)
                {
                    CoopMod.Logger.LogInfo(
                        "[JoinerProfile] Reconciled Gerry shovel with host save: " +
                        $"host_has_shovel={hostShovel.IsPresent}, " +
                        $"profile_copies_removed={removedProfileShovels}, " +
                        $"restored_from=" +
                        $"{(hostShovel.IsPresent ? (profileShovel.IsPresent ? "profile" : "host") : "none")}");
                }
            ...
            private static SharedStoryToolSnapshot CaptureSharedStoryTool(
                Item data,
                string itemId)
            {
                SharedStoryToolSnapshot snapshot =
                    CaptureSharedStoryTool(
                        data?.secondary_inventory,
                        itemId,
                        true);
            ...
            {
                if (items == null)
                    return default(SharedStoryToolSnapshot);
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    return new SharedStoryToolSnapshot
                    {
                        ItemJson = item.ToJSON(0),
                        InToolbelt = inToolbelt,
                        Index = i
                    };
                }
                return default(SharedStoryToolSnapshot);
            ...
            private static int RemoveItemsById(
                List<Item> items,
                string itemId,
                int depth)
            {
            ...
            private static void RestoreSharedStoryTool(
                Item data,
                SharedStoryToolSnapshot snapshot)
            {
                if (data == null || !snapshot.IsPresent)
                    return;

                List<Item> destination = snapshot.InToolbelt
                    ? data.secondary_inventory
                    : data.inventory;
                if (destination == null)
                {
                    destination = new List<Item>();
                    if (snapshot.InToolbelt)
                        data.secondary_inventory = destination;
                    else
                        data.inventory = destination;
                }
                ...
                if (destination[index] != null &&
                    !destination[index].IsEmpty())
                {
                    index = FindEmptyItemSlot(destination);
                }
                if (index < 0)
                {
                ...
                destination[index] =
                    DeserializeItem(snapshot.ItemJson) ?? new Item();
            }

            private static int FindEmptyItemSlot(List<Item> items)
            {
                if (items == null)
                    return -1;
                for (int i = 0; i < items.Count; i++)
            ...
            private static void ClearEquippedItem(
                GameSave save,
                string itemId)
            {
                if (save?.equipped_items == null)
                    return;
                for (int i = 0; i < save.equipped_items.Length; i++)
                {
                    if (string.Equals(
                            save.equipped_items[i],
                            itemId,
                            StringComparison.Ordinal))
                    {
                        save.equipped_items[i] = string.Empty;
                    }
                }
            }
