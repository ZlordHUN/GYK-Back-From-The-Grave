...
namespace GraveyardKeeperCoop.Multiplayer
{
    ...
    public class GameLoadSync : MonoBehaviour
    {
        ...
        public void StartWaitingForPlayers(int playerCount)
        {
            ...
            CosmeticsSync.Instance?.BeginLoadingSynchronization();
            ...
            CreateSyncOverlay();
            UpdateStatusText("Loading world and keeper data");
        }
        ...
        public void BeginClientSaveTransferPresentation()
        {
            ...
            CaptureNativeLoadingLabel();
            UpdateOverlayBackground();
            UpdateStatusText("Requesting world and keeper data");
            UpdateProgressBar();
        }
        ...
        {
            if (isWaitingForSync)
                UpdateStatusText("Sending world and keeper data");
        }
        ...
        private bool IsPostLoadWarmupReady(out string waitingFor)
        {
            ...
            CosmeticsSync cosmeticsSync = CosmeticsSync.Instance;
            if (cosmeticsSync != null &&
                !cosmeticsSync.IsLoadingSynchronizationReady(
                    expectedRemotePlayers,
                    out int receivedCosmetics))
            {
                waitingFor =
                    $"Synchronizing keepers ({receivedCosmetics}/{expectedRemotePlayers})";
                return false;
            }
        ...
        {
            CosmeticsSync.Instance?.CompleteLoadingSynchronization();
        ...
        {
            isInIntroPhase = false;
            Network.OnlineCoopManager.Instance?.EndRedEyeIntroFormation(
                "intro phase ended");
