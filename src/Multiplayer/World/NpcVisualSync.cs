...
namespace GraveyardKeeperCoop.Multiplayer
{
    ...
    public class NpcVisualSync : SyncBehaviour
    {
        ...
        private bool ghostDepthCaptureLogged;
        private bool ghostDepthApplyLogged;
        ...
        private void ResetState()
        {
            foreach (TargetState state in targetStates.Values)
            {
                if (state?.PuppetEpoch != 0U)
                    RestoreLocalDepthSorting(state.Wgo);
            }
            sourceStates.Clear();
            targetStates.Clear();
            sourceScanIndex = 0;
            ...
            ghostDepthCaptureLogged = false;
            ghostDepthApplyLogged = false;
            ResetDiagnostics();
        }
        ...
        private void LateUpdate()
        {
            ...
            foreach (var pair in targetStates)
            {
                ...
                bool needsGhostDepthDiagnostic =
                    ownsMovement &&
                    !ghostDepthApplyLogged &&
                    string.Equals(
                        state.Wgo.obj_id,
                        "npc_ghost",
                        StringComparison.OrdinalIgnoreCase);
                if ((current - target).sqrMagnitude > 0.0001f ||
                    needsGhostDepthDiagnostic)
                {
            ...
            ChunkedGameObject chunk = null)
        {
            ...
            if (ownsMovement)
            {
                ...
                // PlaceAtPos intentionally preserves the receiver's old Z. During
                // a cutscene puppet epoch the source owns depth sorting as well as
                // XY, so force its exact root Z until the epoch is released.
                try
                {
                    RoundAndSortComponent sorter = wgo.round_and_sort;
                    if (sorter != null)
                    {
                        sorter.SetZ(position.z);
                        sorter.MarkPositionDirty();
                        sorter.DoUpdateStuff(true);
                    }
                    if (wgo.transform != null)
                    {
                        Vector3 applied = wgo.transform.position;
                        applied.z = position.z;
                        wgo.transform.position = applied;
                    }
                }
                catch { }
            }
            ...
            if (ownsMovement &&
                ...
                string.Equals(
                    ...
                    StringComparison.OrdinalIgnoreCase))
            {
                ghostDepthApplyLogged = true;
                LogGhostDepthDiagnostic(
                    "authoritative apply",
                    wgo,
                    position);
            }
        ...
        private static void RestoreLocalDepthSorting(WorldGameObject wgo)
        {
            if (wgo == null)
                return;
            ...
            bool reliable = false)
        {
            ...
            // PuppetEpoch is payload-global: sending epoch zero here would tell the
            // receiver that the active cinematic epoch ended, while sending the
            // active value would freeze/depth-own an unrelated NPC. Defer this lossy
            // one-shot instead; the normal post-cutscene broadcast will recover it.
            if (activePuppetEpoch != 0U && !IsLocalCutsceneNpcActor(wgo))
            {
                CoopMod.Logger.LogDebug(
                    $"{LogPrefix} Deferred unrelated immediate state for " +
                    $"{wgo.obj_id} until cutscene epoch {activePuppetEpoch} ends");
                return;
            }
            ...
            out int npcCandidateCount)
        {
                   ...
                   result.Count < MaxNpcsPerPacket)
            {
                ...
                result.Add(entry);
                if (activePuppetEpoch != 0U &&
                    !ghostDepthCaptureLogged &&
                    string.Equals(
                        wgo.obj_id,
                        "npc_ghost",
                        StringComparison.OrdinalIgnoreCase))
                {
                    ghostDepthCaptureLogged = true;
                    LogGhostDepthDiagnostic(
        ...
        private void LogGhostDepthDiagnostic(
            string phase,
            WorldGameObject wgo,
            Vector3 networkPosition)
        {
            ...
            Transform character =
                VisualSyncHelpers.FindChildByName(wgo.transform, "character");
            SpriteRenderer sprite =
                wgo.GetComponentInChildren<SpriteRenderer>(true);
            string spriteState = sprite == null
                ? "none"
                : $"name='{sprite.gameObject.name}', enabled={sprite.enabled}, " +
                  $"active={sprite.gameObject.activeInHierarchy}, " +
                  $"bounds_center={sprite.bounds.center}, " +
                  $"bounds_size={sprite.bounds.size}";
            CoopMod.Logger.LogInfo(
                $"{LogPrefix} Ghost depth diagnostic ({phase}): " +
                $"sent={networkPosition}, root={wgo.transform.position}, " +
                $"character_local=" +
                $"{(character != null ? character.localPosition.ToString() : "none")}, " +
                $"sprite={spriteState}");
        }
        ...
        private void ApplyEntry(NpcEntry entry, Dictionary<string, Sprite> sprites)
        {
            ...
            else if (entry.PuppetEpoch == receivedPuppetEpoch &&
                     IsClientPuppetWindowActive())
            {
                // Active-epoch payloads are already restricted to explicitly
                // enrolled cutscene actors. Depth and stationary poses need the
                // same source ownership as actors that are visibly walking.
    ...
    /// writer for actors enrolled in the current authoritative cutscene epoch.
